...
(function() {
    ...
    const flashcardsTermDef = document.getElementsByClassName("terms-definitions")[0];
    const flashcardsDefTerm = document.getElementsByClassName("definitions-terms")[0];

    const button = document.querySelector('#changeFlashcards');
    const termsDefinitions = document.querySelector('#terms-definitions');
    const definitionsTerms = document.querySelector('#definitions-terms');
    console.log(termsDefinitions);
    console.log(definitionsTerms);

    button.addEventListener('click', function (){
        console.log('Button clicked!');
        console.log(termsDefinitions);
        console.log(definitionsTerms);
        if (definitionsterms.style.display === "none") {
            termsDefinitions.style.display = 'none';
            definitionsTerms.style.display = 'block';
            console.log('if')
        } else {
            termsDefinitions.style.display = 'block';
            definitionsTerms.style.display = 'none';
            console.log('else')
        }
    });
    ...
    glossaryArray.forEach(divMakerTermDefinition);
    glossaryArray.forEach(divMakerDefinitionTerm);

    function divMakerTermDefinition(terms){
        ...
        flashcardsTermDef.appendChild(div);
    }

    function divMakerDefinitionTerm(terms){
        let div = document.createElement("div");
        let definition = document.createElement("h2");
        let term = document.createElement("h2");
        ...
        definition.setAttribute("style", "border-top: 1px solid red; padding: 15px; margin-top: 30px");
        definition.innerHTML = terms.definition;

        term.setAttribute("style", "text-align: center; display: none; color: red")
        term.innerHTML = terms.term;
        ...
        flashcardsDefTerm.appendChild(div);
    }
